...
function generalLocal(st, msg, lang) {
  ...
  return {
    text: lang === "ar"
      ? `أنا معك خطوة بخطوة 💡 وضعك: متاح ${fmt(st.available)} ر.س، مدّخرات ${fmt(st.savings)} ر.س.\nوضّح أكثر: توزيع المكافأة، توفير لهدف، استثمار، أو مبلغ زائد؟`
      : `I'm with you 💡 Your status: ${fmt(st.available)} ر.س available, ${fmt(st.savings)} ر.س saved.\nTell me more: split the stipend, save for a goal, invest, or a shortfall?`,
  };
}
...
export default function App() {
  ...
  return (
    <Ctx.Provider value={value}>
      {screen === "assess" ? <Assessment /> : screen !== "app" ? <Marketing /> : (
      <div style={{ fontFamily: "'IBM Plex Sans Arabic',system-ui,sans-serif", background: appBg, minHeight: "100dvh", display: "flex", justifyContent: "center", color: c.text, transition: "background .3s" }}>
        <div dir={dir} style={{ width: "100%", height: "100dvh", position: "relative", overflow: "hidden", color: c.text, display: "flex", flexDirection: "column", background: appBg, transition: "background .3s" }}>
          <div className="wscroll" style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{ width: "100%", maxWidth: maxW, padding: "calc(env(safe-area-inset-top,0px) + 14px) 18px 104px", ...(tab === "ai" ? { flex: 1, minHeight: 0, display: "flex", flexDirection: "column" } : {}) }}>
              {tab === "home" && <HomeScreen />}
          ...
          {toast && <div style={{ position: "absolute", bottom: 108, left: "50%", transform: "translateX(-50%)", background: c.accent, color: c.onAccent, padding: "10px 20px", borderRadius: 999, fontSize: 13.5, fontWeight: 700, animation: "wPop .25s ease both", whiteSpace: "nowrap", zIndex: 60, maxWidth: "88%", textAlign: "center" }}>{riyalText(toast)}</div>}
        </div>
      </div>
      )}
    </Ctx.Provider>
    ...
    <div dir={dir} style={wrap}><AsTopBar />
      <div style={body}><div style={{ ...inner, gap: 14 }}>
        <div style={{ textAlign: "center", animation: "wPop .4s ease both" }}>
          ...
          <div style={{ fontSize: 12.5, color: c.muted }}>{A.yourPersona}</div>
    ...
    <div>
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "clamp(32px,6vw,70px) 20px", display: "flex", flexDirection: narrow ? "column" : "row", alignItems: "center", gap: "clamp(28px,5vw,50px)" }}>
        <div style={{ flex: 1, textAlign: narrow ? "center" : "start" }}>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 26, flexWrap: "wrap", justifyContent: narrow ? "center" : "flex-start" }}>
            <button onClick={() => setScreen("assess")} style={mkBtn(c.accent, c.onAccent)}>{s.mk.ctaPrimary}</button>
      ...
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "clamp(20px,4vw,40px) 20px clamp(40px,6vw,70px)" }}>
        <div style={{ borderRadius: 28, padding: "clamp(30px,5vw,56px) 24px", textAlign: "center", background: `linear-gradient(135deg, ${c.accent}, ${c.accentText})`, color: c.onAccent }}>
          ...
          <button onClick={() => setScreen("assess")} style={{ ...mkBtn("#fff", c.accent), marginTop: 22 }}>{s.mk.ctaBtn}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ animation: "wUp .5s ease both" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 16 }}>
        ...
        <EntryCard emoji="🏛️" title={s.plat.title} sub={s.plat.sub} onClick={() => setOverlay("platform")} />
...
function ProgressCard() {
  ...
  return (
    <div style={{ background: c.card, border: `1px solid ${c.line}`, borderRadius: 24, padding: 18 }}>
      ...
      <div style={{ fontSize: 11.5, color: c.muted, marginBottom: 14 }}>{s.progScoreUp}: 58 → 76</div>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 10, height: 90 }}>
        {weeks.map((w, i) => (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            <div style={{ fontSize: 10.5, fontWeight: 700, color: c.accent }}>{w.score}</div>
            <div style={{ width: "100%", maxWidth: 34, height: `${(w.score / maxScore) * 62}px`, background: `linear-gradient(180deg, ${c.accent}, ${c.accentText})`, borderRadius: 8, transition: "height .5s" }} />
            <div style={{ fontSize: 10, color: c.muted }}>{s.progWeekLabel(i + 1)}</div>
            <div style={{ fontSize: 9.5, color: c.green }}>✓{w.done}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, height: "100%", animation: "wUp .5s ease both" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "10px 2px 14px" }}>
        ...
        <div><div style={{ fontWeight: 700, fontSize: 17 }}>{s.askWaey}</div><div style={{ fontSize: 11.5, color: c.muted }}>{s.advisorSub}</div></div>
      </div>
...
/* ===================== المزيد ===================== */
function MoreScreen() {
  const { c, s, lang, setLang, points, setPoints, theme, setTheme, setDemoPay, flash, setTab, setScreen } = useCtx();
