...
const L = {
  ar: {
    ...
    dash: {
      ...
      unitK: "ألف", unitM: "مليون", uniList: ["جامعة جدة", "الملك عبدالعزيز", "أم القرى", "الملك سعود", "الإمام"],
    },
  ...
  en: {
    ...
    dash: {
      ...
      unitK: "K", unitM: "M", uniList: ["Univ. of Jeddah", "King Abdulaziz", "Umm Al-Qura", "King Saud", "Imam"],
    },
  ...
  return (
    <RoleShell title={s.role.uni} sub={P.uniDash} onBack={() => setScreen("role")}>
      ...
      {tab === 0 && (<>
        ...
        <div style={{ background: c.card, border: `1px solid ${c.line}`, borderRadius: 20, padding: 18, marginBottom: 14 }}>
          <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 12, color: c.accentText }}>{D.awareness} · {D.trend}</div>
          <MiniBars data={trend} c={c} col={c.accent} />
        </div>
        <div style={{ background: c.card, border: `1px solid ${c.line}`, borderRadius: 20, padding: 18, marginBottom: 14 }}>
          <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 14, color: c.accentText }}>{D.spendHeat}</div>
...
function BankDashScreen() {
  ...
  const rankBase = [[s.dash.uniList[0], [1240, 82, 71]], [s.dash.uniList[1], [1180, 78, 68]], [s.dash.uniList[2], [960, 74, 64]], [s.dash.uniList[3], [1090, 70, 66]], [s.dash.uniList[4], [820, 66, 61]]];
  const regionFilter = region === 0 ? rankBase : rankBase.filter((_, i) => (region === 1 ? i % 3 === 0 : region === 2 ? i % 3 === 1 : i % 3 === 2));
  const ranking = (regionFilter.length ? regionFilter : rankBase).map(([n, v]) => ({ n, v: v[rankBy] })).sort((a, b) => b.v - a.v);
  ...
  return (
    <RoleShell title={s.role.bank} sub={P.bankDash} onBack={() => setScreen("role")}>
      ...
      {tab === 0 && (<>
        ...
        <div style={{ background: c.card, border: `1px solid ${c.line}`, borderRadius: 20, padding: 18, marginBottom: 14 }}>
          <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 12, color: c.accentText }}>{D.ageGroup}</div>
